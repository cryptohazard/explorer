...
func (q *QtumRPC) GetBlockHash(blockNumber int) (string, error) {
	ret, err := q.Call("getblockhash", []interface{}{blockNumber})
	if err != nil {
		return "", err
	}
...
func (q *QtumRPC) GetBlock(blockNumber int) (Block, error) {

	hash, err := q.GetBlockHash(blockNumber)
	if err != nil {
		return Block{}, err
	}
	ret, err := q.Call("getblock", []interface{}{hash})
	if err != nil {
		return Block{}, err
	}
	b := Block{}
	errwan := mapstructure.Decode(ret, &b)
	if errwan != nil {
		return Block{}, errwan
	}
